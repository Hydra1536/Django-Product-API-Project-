from django.test import TestCase
from django.contrib.auth import get_user_model
from django.contrib.auth.models import AnonymousUser
from rest_framework.test import APIRequestFactory
from product_api.permissions import ProductPermissions

User = get_user_model()


class ProductPermissionsTest(TestCase):
    def setUp(self):
        self.factory = APIRequestFactory()
        self.admin_user = User.objects.create_user(
            username="admin",
            email="admin@example.com",
            password="pass",
            role="admin"
        )
        self.staff_user = User.objects.create_user(
            username="staff",
            email="staff@example.com",
            password="pass",
            role="staff"
        )
        self.super_staff_user = User.objects.create_user(
            username="superstaff",
            email="superstaff@example.com",
            password="pass",
            role="staff",
            is_super_stuff=True
        )
        self.customer_user = User.objects.create_user(
            username="customer",
            email="customer@example.com",
            password="pass",
            role="customer"
        )

    def test_admin_has_full_access(self):
        permission = ProductPermissions()
        request = self.factory.get("/")
        request.user = self.admin_user
        self.assertTrue(permission.has_permission(request, None))

        request = self.factory.post("/")
        request.user = self.admin_user
        self.assertTrue(permission.has_permission(request, None))

    def test_super_staff_has_full_access(self):
        permission = ProductPermissions()
        request = self.factory.get("/")
        request.user = self.super_staff_user
        self.assertTrue(permission.has_permission(request, None))

        request = self.factory.post("/")
        request.user = self.super_staff_user
        self.assertTrue(permission.has_permission(request, None))

    def test_staff_has_read_only_access(self):
        permission = ProductPermissions()
        request = self.factory.get("/")
        request.user = self.staff_user
        self.assertTrue(permission.has_permission(request, None))

        request = self.factory.post("/")
        request.user = self.staff_user
        self.assertFalse(permission.has_permission(request, None))

    def test_customer_has_read_only_access(self):
        permission = ProductPermissions()
        request = self.factory.get("/")
        request.user = self.customer_user
        self.assertTrue(permission.has_permission(request, None))

        request = self.factory.post("/")
        request.user = self.customer_user
        self.assertFalse(permission.has_permission(request, None))

    def test_unauthenticated_user_denied(self):
        permission = ProductPermissions()
        request = self.factory.get("/")
        request.user = AnonymousUser()
        self.assertFalse(permission.has_permission(request, None))

    def test_unknown_role_denied(self):
        permission = ProductPermissions()
        user = User.objects.create_user(
            username="unknown",
            email="unknown@example.com",
            password="pass",
            role="unknown"
        )
        request = self.factory.get("/")
        request.user = user
        self.assertFalse(permission.has_permission(request, None))
