...
class UserAPITestCase(APITestCase):
    ...
    def test_admin_can_list_users(self):
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {self.get_token(self.admin_user)}"
        )
    ...
    def test_staff_cannot_list_users(self):
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {self.get_token(self.staff_user)}"
        )
        response = self.client.get(self.users_list_url)
    ...
    def test_customer_cannot_list_users(self):
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {self.get_token(self.customer_user)}"
        )
        response = self.client.get(self.users_list_url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_change_password(self):
        ...
        url = reverse(
            "accounts:users-change-password", kwargs={"pk": self.staff_user.pk}
        )
